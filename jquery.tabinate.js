$.fn.tabinate = function(options) { // v0.4
...
// Default Settings (don't adjust here!)
	...
	var container = this;
	var tabs = container.find('.'+settings.tabClass);
	...
	tabs.removeClass(settings.activeTabClass).hide(0, function() {
		...
		if($('#'+hash).length > 0) { // If hash, show tab
			...
			$('#'+hash+'.'+settings.tabClass).show(0, function() {
				container.find("ul:eq(0) a[href='#"+hash+"']:eq(0)").addClass(settings.activeLinkClass);
			});
		}
		if(container.find('.'+settings.tabClass+':visible').length == 0){ // If nothing visible, show first
			var links = container.find('ul:eq(0) li:eq(0) a:eq(0)').addClass(settings.activeLinkClass);
			tabs.eq(0).show().addClass(settings.activeTabClass);
		}
	});

	// Tab Click
	container.find('ul:eq(0) li a').click(function() {
		container.find('ul:eq(0) a').not($(this)).removeClass(settings.activeLinkClass);
		...
		if($(targetdiv).length > 0) { // if using ID href
			...
			eq = container.find('ul:eq(0) li a').index(this);
			tabs.removeClass(settings.activeTabClass).hide(0, function() {
